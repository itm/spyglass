...
/**
 * Abstract class that represents a drawing object. <p/> <p> A drawing object has a lifetime, bounded by the calls to
 * {@link DrawingObject#init(DrawingArea)} and {@link DrawingObject#destroy()}, respectively. During this time, the
 * drawingObject is bounded to a drawingArea. A drawingObject can have only one life, i.e. once it has been destroyed,
 * it cannot be initialized again. </p> <p/> <p> All DrawingObjects are guaranteed to be thread-safe. (Implementors of
 * subclasses should make sure of this too!) </p> <p/> <p> Every drawing object has a bounding box. The bounding box is
 * a rectangle describing the boundaries of the object in absolute coordinates. The bounding box is used to decide
 * whether a drawing object is inside the part of the screen which needs to be repainted at given time. Because of this,
 * it is of vital importance that the bounding box is at all times in sync with the "image", which will be drawn by from
 * {@link #drawObject(GC)}. </p> <p/> <p> The bounding box of the drawing object may change over time, e.g. if the
 * position of the drawing object is changed. Because of this, there is the dilemma when to update the bounding box to
 * bring it back to sync with the content of the drawing object. Unfortunately calculating the bounding box may require
 * access to the SWT thread, so we may run into deadlocking issues if we tried to update the bounding box synchronously
 * every time some property (e.g. the position) of the drawing object has been modified. </p> <p/> <p> So to avoid
 * deadlocks we have to recalculate the bounding box asynchronously. This is done by marking the bounding box dirty
 * whenever it needs to be updated and then sending a signal to the PluginController to recalculate the bounding box at
 * the next opportunity. </p> <p/> <p> But now that the bounding box is updated asynchronously the bounding box may
 * become out of sync for periods of time with the "image" that ends up to be drawn on the screen by {@link
 * #drawObject(GC)}. How can we avoid that? By keeping a "shadow copy" of the drawing object which is used for actual
 * painting. This works like this: We always keep a copy ourself (a {@link #clone()} of the drawing object) held back in
 * {@link DrawingObject#shadowCopy}. Usually this shadowCopy is always kept in sync with the master object (see {@link
 * DrawingObject#fireContentChangedEvent()}). All drawing operations are not done on the master object, but in fact on
 * this shadow copy (see {@link #drawObject(GC)}. In the case the bounding box get dirtied, we then stop updating the
 * shadow copy, to let it stay on the last state known to be in sync with the bounding box. Only later, when out {@link
 * PluginController} has updated our bounding box, we sync the shadow copy again. </p> <p/> <p> To avoid graphical
 * errors during the periods where the shadow copy is out of sync with the master object, it is important, that {@link
 * #clone()} is able to make clean copies of the object which result in independent data structures. E.g. if the drawing
 * object would store parts of its data in an object which is not cloned by {@link #clone()}, then both the master
 * object and the shadow copy would reference the same object. This could result in short time graphical errors. </p>
 *
...
public abstract class DrawingObject implements Cloneable {
	...
	/**
	 * After a property of this DrawingObject has changed, it may not be possible to immediately
	 * update the boundingBox. In such cases we have to do the drawing on a copy of our
	 * drawingObject which is known to be consistent with its boundingBox.
	 */
	 ...
	 *
	 * <p>
	 * Subclasses may overwrite this method if they want to do additional things (e.g. add listener
	 * to the drawing area).
	 * </p>
	 *
	 * @param drawingArea
	...
	/**
	 * Returns a reference to the drawing area. Between the calls to {@link #init(DrawingArea)} and
	 * {@link #destroy()}, this method is guaranteed to return a valid reference.
	 *
	 * <p>
	 * Before {@link #init(DrawingArea)} or after {@link #destroy()} are called, this method will
	 * return null!
	 * </p>
	 *
	...
	/**
	 * In this method the drawingObject should draw itself in the drawingArea. While doing so, it
	 * must not draw beyond the limits imposed by its bounding box.
	 *
	 * @param gc
	 ...
	 *
	 * <p>
	 * The bounding box is kept in sync with the image that is drawn through draw(). It is not
	 * automatically updated whenever a property of a drawing object is modified.
	 * </p>
	 *
	 ...
	 *
	 * <p>
	 * If the bounding box is dirty, at some point later in time the PluginController will
	 * recalculate the bounding box. Until then the old bounding box will remain in place.
	 * </p>
	 *
	 * <p>
	 * This method must be called whenever the bounding box may have been changed.
	 * </p>
	 *
	 */
	protected synchronized final void markBoundingBoxDirty() {

		// The bounding box doesn't matter in these cases
		if (state != State.ALIVE) {
			return;
		}
	...
	/**
	 * Added as a workaround for unit testing. This method does what
	 * {@link de.uniluebeck.itm.spyglass.drawing.DrawingObject#syncBoundingBox()} does but does not check preconditions.
	 */
	public final synchronized void syncBoundingBoxForce() {
	...
	/**
	 * Synchronizes the bounding box and sets {@code isBoundingBoxDirty} to {@code false}.
	 */
	protected final synchronized void syncBoundingBoxInternal() {
	...
	/**
	 * Clones this drawing object. Subclasses have to overwrite this method if a deep-copy is
	 * necessary to avoid that modifications on the original will reflect on the copy.
	 *
	...
	/**
	 * Adds a listener for changes concerning the content of the drawing object (e.g. anything that
	 * would result in a different drawing)
	 *
	 * @param listener
	 *            a listener for changes concerning the object's bounding box
	 */
	...
	/**
	 * Removes a listener for changes concerning the content of the drawing object (e.g. anything
	 * that would result in a different drawing)
	 *
	 * @param listener
	 *            a listener for changes concerning the object's bounding box
	 */
	...
	/**
	 * Adds a listener that is evoked whenever the boundingbox of this drawingobject gets dirty
	 * (i.e. needs to updated).
	 *
	 * <p>
	 * This listener is used solely for the communication between drawingObjects and the
	 * PluginController.
	 * </p>
	 *
	 * @param listener
	...
	/**
	 * Removes a listener that is evoked whenever the boundingbox of this drawingobject gets dirty
	 * (i.e. needs to updated).
	 *
	 * @param listener
	 */
	...
	/**
	 * Informs all listeners that this drawing object has an outdated boundingBox and is in need of
	 * an SWT display thread to get it back up-to-date.
	 *
	 */
