...
public class DataStore extends HashSet<DataPoint> {
	 ...
	 * 
	 * @param k number of points to return (must be at least 1)
	 * @param pos a position
	 * @return the k points nearest to the given position (or less points, if there
	 * are not enough in the store)
	 */
	public List<DataPoint> kNN(final AbsolutePosition pos, final int k) {
		if (k<1) {
			...
			for (final DataPoint dataPoint : this) {
				final double distance = pos.getDistance(dataPoint.position);
				for (int i=0; i<k;i++) {
					if ((list.size()<i+1) || (list.get(i).position.getDistance(pos)>distance))  {
