...
public abstract class DrawingObject {
	...
	/**
	 * The identifier is used to discriminate drawing objects against each other. It is NOT the same
	 * as the identifier of the (physical) object which is represented. But since two different
	 * physical object types can share an identifier, this synthetic identifier is needed to provide
	 * uniqueness for each drawing object.
	 */
	private int id = 0;
	...
	// --------------------------------------------------------------------------------
	/**
	 * Constructor generating the objects identifier.<br>
	 * The identifier is used to discriminate drawing objects against each other. It is NOT the same
	 * as the identifier of the (physical) object which is represented. But since two different
	 * object types can share an identifier this synthetic identifier is needed for each drawing
	 * object.<br>
	 * The identifier is generated by evaluating the current timestamp which is "cropped" to
	 * integer.
	 */
	public DrawingObject() {
		// TODO: Store the previously generated timestamps to guarantee uniqueness?
		this.id = (int) new Date().getTime();
	}
	
	// --------------------------------------------------------------------------------
	/**
	 * Constructor setting the drawing object's identifier.<br>
	 * The identifier is used to discriminate drawing objects against each other. It is NOT the same
	 * as the identifier of the (physical) object which is represented. But since two different
	 * object types can share an identifier this synthetic identifier is needed for each drawing
	 * object.<br>
